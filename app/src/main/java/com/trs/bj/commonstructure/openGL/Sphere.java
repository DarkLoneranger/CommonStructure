        ...
        for (pai = -90.0f; pai < 90.0f; pai += step) {
            ...
            for (theta = 0.0f; theta <= 360.0f; theta += step) {
                ...
                if (n > 31) {
                    ...
                    //glNormalPointer指明渲染时使用到的法线数组的数据位置。type指明法线坐标的数据类型。
                    // stride指明两个相邻的法线数据间的字节偏移，这将允许顶点数据和顶点属性数据保存在同一个数组中或不同的数组中。
                    // 在某些实现中，单个数组保存的效率会更高。
                    gl.glNormalPointer(GL10.GL_FLOAT, 0, vBuf);
                    //采用顶点数组方式绘制图形时，使用该函数。该函数根据顶点数组中的坐标数据和指定的模式，进行绘制。
